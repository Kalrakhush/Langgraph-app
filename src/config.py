...
class Config:
    """Configuration settings for the AI pipeline using Streamlit secrets."""

    # API Keys
    GROQ_API_KEY: str = st.secrets.get("GROQ_API_KEY", "")
    OPENWEATHERMAP_API_KEY: str = st.secrets.get("OPENWEATHERMAP_API_KEY", "")
    LANGSMITH_API_KEY: str = st.secrets.get("LANGSMITH_API_KEY", "")

    # Qdrant Cloud Settings
    QDRANT_URL: str = st.secrets.get("QDRANT_URL", "")
    QDRANT_API_KEY: str = st.secrets.get("QDRANT_API_KEY", "")
    COLLECTION_NAME: str = "pdf_embeddings"

    # LangSmith Settings
    LANGSMITH_TRACING: bool = st.secrets.get("LANGSMITH_TRACING", "true").lower() == "true"
    LANGSMITH_PROJECT: str = st.secrets.get("LANGSMITH_PROJECT", "ai-pipeline-demo")
